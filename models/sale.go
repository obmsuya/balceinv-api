...
type Sale struct {
	...
	// AmountPaid is the amount tendered at checkout, stored so a receipt
	// printed or reprinted later (POST /api/print/receipt reloads the sale
	// fresh from the DB) can still show Amount Paid / Change — those were
	// previously computed only in-memory at sale creation and discarded.
	AmountPaid float64   `gorm:"column:amount_paid;default:0" json:"amount_paid"`
